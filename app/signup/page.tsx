...
export default function SignupPage() {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (Object.keys(validation).length === 0) {
      setMessage("Signup complete! You can now sign in with your new account.");
    } else {
      setMessage("");
    }
  };
    ...
    <div className="min-h-screen bg-zinc-50 px-6 py-10 text-zinc-900 dark:bg-zinc-950 dark:text-zinc-50">
      <div className="mx-auto max-w-xl rounded-3xl bg-white/90 p-10 shadow-[0_20px_120px_-35px_rgba(15,23,42,0.35)] backdrop-blur-xl dark:bg-zinc-900/95 sm:p-12">
        <div className="mb-8 text-center">
          ...
          <h1 className="mt-4 text-3xl font-semibold sm:text-4xl">Start your Sandigan journey</h1>
          <p className="mt-3 text-sm text-zinc-600 dark:text-zinc-400">
            Complete the form below to create a secure account and access the app.
          </p>
        </div>
        ...
        <p className="mt-8 text-center text-sm text-zinc-600 dark:text-zinc-400">
          Already registered?{' '}
          <Link className="font-semibold text-sky-600 hover:text-sky-500" href="/login">
            Sign in
          </Link>
        </p>
      </div>
